# 大家的游戏攻略 Cocos2d-x导入手册

## 1. 大家的游戏攻略 Cocos2d-x Plugin

### 1-1. 运行环境
 * Cocos2d-x 2.2.x, 3.x以上
 * iOS 6.0 以上
 * Android 4.2.2 以上

### 1-2. 术语
 * **"scene"**  
在SDK中显示的关于各个任务，关卡、活动等的游戏攻略统称为“scene”。

 * **"page"**
贵公司游戏中打开游戏攻略的游戏画面被称为"page"。

## 2. 下载SDK
正在准备

## 3. 导入到项目

### 各种OS的配置
* [iOS项目配置](./ios/README.md)
* [Android项目配置](./android/README.md)

## 4. SDK功能的安装

通过用C++脚本定义好的函数名来调用SDK的功能。

### 4-1. APP启动时

请导入下面的头文件。

```c++
#include "CCOrca.h"
#include <OrcaSDK/OrcaCocos2dx.h>
```

在APP启动时，请设置clientID和applicationID。
鄙公司会另外（调查表等形式）发行并通知给贵公司各种ID。

[安装范例]

```c++
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
  OrcaCocos2dx::configure(
    "a5af3c3f6914c2d7aeb0fb702b940a06", // Client ID
    "0f67abb071b1a4267b4b13e5f47775df"  // Application ID
  );
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
  CCOrca::configure(
    "a5af3c3f6914c2d7aeb0fb702b940a06", // Client ID
    "0f67abb071b1a4267b4b13e5f47775df"  // Application ID
  );
#endif
```

|参数|内容|必须|备注|
|:------|:------|:------|:------|
|第一个参数|ClientID|必須|联系本社发行。<br/>|
|第二个参数|ApplicationID|必須|联系本社发行。<br/>※ iOS和Android通用|

### 4-2. APP启动时（安装后初次启动时）

请按照在安装APP后只执行**初回起動時**处理的方式来安装代码。

[安装范例]

```c++
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
  OrcaCocos2dx::configureFirstStart(
    "a5af3c3f6914c2d7aeb0fb702b940a06", // ClientID
    "0f67abb071b1a4267b4b13e5f47775df"  // ApplicationID
  );
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
  CCOrca::configureFirstStart(
    "a5af3c3f6914c2d7aeb0fb702b940a06", // ClientID
    "0f67abb071b1a4267b4b13e5f47775df"  // ApplicationID
  );
#endif
```

※参数和「4-1. APP启动时」是共通的。

### 4-3. 显示游戏攻略的浮动图标

想在贵公司的APP画面上显示**游戏攻略的浮动图标**时，执行以下步骤。  
pageID、sceneID请联系本社发行。  

[安装范例]

```c++
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
  OrcaCocos2dx::showFloatIcon(
    "d4dc9737ecb40d25d1bc5b8a1f7798d4", // PageID
    "65928b3ceeb3e9cb24d917e5532ad332|afc4a607d40edbe7c51853ac3af0910f",  // sceneID（有多个攻略的情况）
    "等级1",                           // 用户等级
    "15",                                // 用户经验值
    "勇者"                               // 用户角色
    "{'OptionalData':{'puid':'1234','pname':'山田太郎','level':'等级1', ・・・ }}" // 任意项目
  );
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
  CCOrca::showFloatIcon(
    "d4dc9737ecb40d25d1bc5b8a1f7798d4", // PageID
    "65928b3ceeb3e9cb24d917e5532ad332|afc4a607d40edbe7c51853ac3af0910f",  // sceneID（有多个攻略的情况）
    "等级1",                           // 用户等级
    "15",                                // 用户经验值
    "勇者"                               // 用户角色
    "{'OptionalData':{'puid':'1234','pname':'山田太郎','level':'等级1', ・・・ }}" // 任意项目
  );
#endif
```

[安装范例（不设置过滤条件的情况）]

```c++
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
  OrcaCocos2dx::showFloatIcon(
    "d4dc9737ecb40d25d1bc5b8a1f7798d4",  // PageID
    "65928b3ceeb3e9cb24d917e5532ad332",  // sceneID（只有1个攻略的情况）
    "",                                  // 用户等级
    "",                                  // 用户经验值
    "",                                  // 用户角色
    ""                                   // 任意项目
  );
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
  CCOrca::showFloatIcon(
    "d4dc9737ecb40d25d1bc5b8a1f7798d4",  // PageID
    "65928b3ceeb3e9cb24d917e5532ad332",  // sceneID（只有1个攻略的情况）
    "",                                  // 用户等级
    "",                                  // 用户经验值
    "",                                  // 用户角色
    ""                                   // 任意项目
  );
#endif
```

#### 攻略浮动图标的切换

游戏画面切换时，调用函数并指定pageID、sceneID，参考「4-3. 显示游戏攻略的浮动图标」。
已经显示在画面上的图标会被删除，显示新的浮动图标。

#### 自定义攻略浮动图标

准备好调查表和每个page图标的设计、尺寸、图片之后，在本公司的投放系统里做上传。

#### 隐藏攻略浮动图标

想隐藏已经显示在画面上的攻略浮动图标的话，不指定scenceID再执行「4-3. 显示游戏攻略的浮动图标」的步骤。

[安装范例]

```c++
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
  OrcaCocos2dx::showFloatIcon(
    "d4dc9737ecb40d25d1bc5b8a1f7798d4",  // PageID
    "",                                  // sceneID（不指定）
    "",                                  // 用户等级
    "",                                  // 用户经验值
    "",                                  // 用户角色
    ""                                   // 任意项目
  );
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
  CCOrca::showFloatIcon(
    "d4dc9737ecb40d25d1bc5b8a1f7798d4",  // PageID
    "",                                  // sceneID（不指定）
    "",                                  // 用户等级
    "",                                  // 用户经验值
    "",                                  // 用户角色
    ""                                   // 任意项目
  );
#endif
```


|参数|内容|必须|备注|
|:------|:------|:------|:------|
|第１个参数|pageID|必须|鄙公司发行并告知。<br/>※ iOS和Android通用|
|第２个参数|sceneID|可选|鄙公司发行并告知。<br/>多个sceneID的情况下、用" &#124; "间隔（最大20个）<br/>不指定sceneID的情况下，将隐藏攻略浮动图标<br/>※ iOS和Android通用|
|第３个参数|用户等级|可选|以用户等级过滤攻略。<br/>（不设定等情况下，传入空字符串）|
|第４个参数|用户经验值|可选|以用户经验值过滤攻略。<br/>（不设定等情况下，传入空字符串）|
|第５个参数|用户角色|可选|以用户角色过滤攻略。<br/>（不设定等情况下，传入空字符串）|
|第６个参数|任意項目|可选|以json格式传入任意项目。 <br/>（不设定等情况下，传入空字符串）|

【关于任意项目】  
可以设定的项目请参考下面列表。  
也可以追加下面的表格以外的任意项目。如果有希望追加的项目，可联系本公司商谈。  
任意项目里面包含多字节字符的情况下，脚本的编码格式需要设定为UTF-8（带BOM）。

|任意項目名|key|设定值|
|:------|:------|:------|
|用户ID|puid   |游戏中的用户ID|
|用户名|pname  |游戏中的用户名|
|对战boss名|bchar  |游戏中的boss名|
|等级(排名)|level  |用户的等级值或名称|
|经验值|exp  |用户的经验值或者名称|
|角色名|char  |用户游戏角色名称|
|HP|hp  |游戏人物HP|
|体力|stm  |游戏人物体力值|
|攻击力|power  |游戏人物攻击力|
|装备/武器名|item  |游戏角色的武器、装备的名称|

※各項目最大输入128字符。


### 4-4. App后台运行和重新转入前台运行

App后台运行和重新转入前台运行时，控制画面上攻略浮动图标的显示。

转入后台运行或转入前台运行的时候，需要重新调用SDK函数。

[安装范例]

AppDelegate.h
```c++
#include "CCOrca.h"
```

AppDelegate.cpp

```c++
#if (CC_TARGET_PLATFORM ==CC_PLATFORM_IOS)
#include "OrcaSDK/OrcaCocos2dx.h"
#endif
```

```c++
void AppDelegate::applicationDidEnterBackground() {
    CCDirector::sharedDirector()->stopAnimation();

    // 添加下面的转入后台处理
    #if (CC_TARGET_PLATFORM ==CC_PLATFORM_IOS)
        OrcaCocos2dx::setBackground(true);
    #elif (CC_TARGET_PLATFORM ==CC_PLATFORM_ANDROID)
        CCOrca::pause();
    #endif
}

void AppDelegate::applicationWillEnterForeground() {
    CCDirector::sharedDirector()->startAnimation();

    // 添加下面的转入前台处理
    #if (CC_TARGET_PLATFORM ==CC_PLATFORM_IOS)
        OrcaCocos2dx::setBackground(false);
    #elif (CC_TARGET_PLATFORM ==CC_PLATFORM_ANDROID)
        CCOrca::resume();
    #endif
}
```

## 5. SDK导入后测试

### 5-1. 确认事项

申请上线前，请测试导入的SDK的状态，以及各功能是否可以正常运行。

1. 用测试机启动导入SDK的App
1. 画面转移到调用 **游戏攻略** 地点（Scene或关卡）的画面
1. 显示 **浮动图标**
1. 点击 **浮动图标** 后显示游戏攻略
1. 关闭 **攻略情報** 画面

请通知鄙公司测试日期。我们将通过检查日志文件确认SDK的功能有没有正常被调用。  
鄙公司确认之后，没有问题后，测试结束。

### 5-2. 关于调试模式

我们提供了测试时确认用的调试模式

【调试模式功能】

* 不进行播放控制、播放测试网页及测试视频。  
* 把开发环境的标准输出写入日志。

|调试模式scene|sceneID|
|:------|:------|
|scene1|fc7f24abc577f37d44b6fb0095dcd976|
|scene2|aee4857751950eee5441057d436e506a|
|scene3|ea0a160b2a496c33efe0c9511f61b34f|
|scene4|c0f7d3256b91dc80faab2d0ca34696e6|
|scene5|a3c9cc0625a56a2f9abb1400f2e17f80|
|scene6|46d8ca247c7cdd3f81c0649b1bc3c2fc|
|scene7|f576372b68c175cb52730f05a510eb86|
|scene8|3ae0fa5850a7ef180b56617f271124a7|
|scene9|a353d25ddc95549b6bb848fbf6401510|
|scene10|a63b500d519653e2306c77213ef91eb9|

测试结束后，鄙公司将解除SDK的调试模式，变成运营模式。

----

导入SDK的时候，如果有什么问题，请发送邮件到下面的地址咨询。

咨询邮箱：z-mediadev@cyber-z.co.jp
